 ...
 */
static void shadow_store(struct r82xx_priv *priv, u8int reg, const u8int *val,
			 int len)
{
...
static int r82xx_write(struct r82xx_priv *priv, u8int reg, const u8int *val,
		       unsigned int len)
{
	int rc, size, pos = 0;
	...
	do {
		...
		if (rc != size + 1) {
			fprint(2, "%s: i2c wr failed=%d reg=%02x len=%d\n",
				   argv0, rc, reg, size);
			if (rc < 0)
				return rc;
			return -1;
...
static int r82xx_write_reg(struct r82xx_priv *priv, u8int reg, u8int val)
{
...
static int r82xx_write_reg_mask(struct r82xx_priv *priv, u8int reg, u8int val,
				u8int bit_mask)
{
...
static u8int r82xx_bitrev(u8int byte)
{
	const u8int lut[16] = { 0x0, 0x8, 0x4, 0xc, 0x2, 0xa, 0x6, 0xe,
				  0x1, 0x9, 0x5, 0xd, 0x3, 0xb, 0x7, 0xf };

	return (lut[byte & 0xf] << 4) | lut[byte >> 4];
}

static int r82xx_read(struct r82xx_priv *priv, u8int reg, u8int *val, int len)
{
	int rc, i;
	u8int *p = &priv->buf[1];
	...
	if (rc != len) {
		fprint(2, "%s: i2c rd failed=%d reg=%02x len=%d\n",
			   argv0, rc, reg, len);
		if (rc < 0)
			return rc;
		return -1;
...
static int r82xx_set_mux(struct r82xx_priv *priv, u32int freq)
{
...
static int r82xx_set_pll(struct r82xx_priv *priv, u32int freq)
{
	int rc, i;
	unsigned sleep_time = 10000;
	u64int vco_freq;
	u32int vco_fra;	/* VCO contribution by SDM (kHz) */
	u32int vco_min = 1770000;
	u32int vco_max = vco_min * 2;
	u32int freq_khz, pll_ref, pll_ref_khz;
	u16int n_sdm = 2;
	u16int sdm = 0;
	u8int mix_div = 2;
	u8int div_buf = 0;
	u8int div_num = 0;
	u8int vco_power_ref = 2;
	u8int refdiv2 = 0;
	u8int ni, si, nint, vco_fine_tune, val;
	u8int data[5];
	...
	vco_freq = (u64int)freq * (u64int)mix_div;
	nint = vco_freq / (2 * pll_ref);
	vco_fra = (vco_freq - 2 * pll_ref * nint) / 1000;

	if (nint > ((128 / vco_power_ref) - 1)) {
		fprint(2, "[R82XX] No valid PLL values for %u Hz!\n", freq);
		return -1;
	}
...
static int r82xx_sysfreq_sel(struct r82xx_priv *priv, u32int freq,
			     enum r82xx_tuner_type type,
			     u32int delsys)
{
	int rc;
	u8int mixer_top, lna_top, cp_cur, div_buf_cur, lna_vth_l, mixer_vth_l;
	u8int air_cable1_in, cable2_in, pre_dect, lna_discharge, filter_cur;
...
{
	int rc, i;
	u32int if_khz, filt_cal_lo;
	u8int data[5];
	u8int filt_gain, img_r, filt_q, hp_cor, ext_enable, loop_through;
	u8int lt_att, flt_ext_widest, polyfil_cur;
...
int r82xx_set_bandwidth(struct r82xx_priv *priv, int bw, u32int rate)
{
	int rc;
	unsigned int i;
	int real_bw = 0;
...
{
	int rc = -1;
	u32int lo_freq = freq + priv->int_freq;
	u8int air_cable1_in;
