...
static int fc0013_writereg(void *dev, u8int reg, u8int val)
{
	u8int data[2];
	data[0] = reg;
	data[1] = val;
...
static int fc0013_readreg(void *dev, u8int reg, u8int *val)
{
	u8int data = reg;
...
int fc0013_rc_cal_add(void *dev, int rc_val)
{
	...
	if (val > 15)
		...
		ret = fc0013_writereg(dev, 0x10, (u8int)val);
...
static int fc0013_set_vhf_track(void *dev, u32int freq)
{
	int ret;
	u8int tmp;
...
{
	int i, ret = 0;
	u8int reg[7], am, pm, multi, tmp;
	u64int f_vco;
	u32int xtal_freq_div_2;
	u16int xin, xdiv;
	int vco_select = 0;
	...
	xdiv = (u16int)(f_vco / xtal_freq_div_2);
	...
	if ((reg[1] > 15) || (reg[2] < 0x0b)) {
		fprint(2, "[FC0013] no valid PLL combination "
				"found for %u Hz!\n", freq);
		return -1;
	}
	...
	xin = (u16int)((f_vco - (f_vco / xtal_freq_div_2) * xtal_freq_div_2) / 1000);
