...
u32int unsigned_delta(u32int a, u32int b)
{
	if (a > b)
		return a - b;
	else
 ...
 */
static int e4k_reg_write(struct e4k_state *e4k, u8int reg, u8int val)
{
	int r;
	u8int data[2];
	data[0] = reg;
 ...
 */
static int e4k_reg_read(struct e4k_state *e4k, u8int reg)
{
	u8int data = reg;
 ...
 *  \returns 0 on success, negative in case of error
 */
static int e4k_reg_set_mask(struct e4k_state *e4k, u8int reg,
		     u8int mask, u8int val)
{
	u8int tmp = e4k_reg_read(e4k, reg);
 ...
 *  \returns 0 on success, negative in case of error
 */
static int e4k_field_write(struct e4k_state *e4k, const struct reg_field *field, u8int val)
{
	int rc;
	u8int mask;
...
static const u32int rf_filt_center_uhf[] = {
...
static const u32int rf_filt_center_l[] = {
...
static int closest_arr_idx(const u32int *arr, unsigned int arr_size, u32int freq)
{
	unsigned int i, bi = 0;
	u32int best_delta = 0xffffffff;

	/* iterate over the array containing a list of the center
	 * frequencies, selecting the closest one */
	for (i = 0; i < arr_size; i++) {
		u32int delta = unsigned_delta(freq, arr[i]);
		if (delta < best_delta) {
			best_delta = delta;
			bi = i;
		}
...
static int choose_rf_filter(enum e4k_band band, u32int freq)
{
	int rc;
...
/* Mixer Filter */
static const u32int mix_filter_bw[] = {
...
/* IF RC Filter */
static const u32int ifrc_filter_bw[] = {
...
/* IF Channel Filter */
static const u32int ifch_filter_bw[] = {
...
static const u32int *if_filter_bw[] = {
	mix_filter_bw,
	ifch_filter_bw,
	ifrc_filter_bw,
};

static const u32int if_filter_bw_len[] = {
...
static int find_if_bw(enum e4k_if_filter filter, u32int bw)
{
...
static int is_fosc_valid(u32int fosc)
{
	if (fosc < MHZ(16) || fosc > MHZ(30)) {
		fprint(2, "[E4K] Fosc %u invalid\n", fosc);
		return 0;
	}
...
static int is_z_valid(u32int z)
{
	if (z > 255) {
		fprint(2, "[E4K] Z %u invalid\n", z);
		return 0;
	}
...
static int use_3ph_mixing(u32int flo)
{
...
static u64int compute_fvco(u32int f_osc, u8int z, u16int x)
{
	u64int fvco_z, fvco_x, fvco;
	 ...
	 */
	fvco_z = (u64int)f_osc * z;

	if (!is_fvco_valid(fvco_z))
		return 0;

	fvco_x = ((u64int)f_osc * x) / E4K_PLL_Y;
...
static u32int compute_flo(u32int f_osc, u8int z, u16int x, u8int r)
{
	u64int fvco = compute_fvco(f_osc, z, x);
	if (fvco == 0)
		return -1;
...
u32int e4k_compute_pll_params(struct e4k_pll_params *oscp, u32int fosc, u32int intended_flo)
{
	u32int i;
	u8int r = 2;
	u64int intended_fvco, remainder;
	u64int z = 0;
	u32int x;
	int flo;
	...
	intended_fvco = (u64int)intended_flo * r;
...
int e4k_tune_freq(struct e4k_state *e4k, u32int freq)
{
	...
	if (!(rc & 0x01)) {
		fprint(2, "[E4K] PLL not locked for %u Hz!\n", freq);
		return -1;
	}
...
static const s8int if_stage1_gain[] = {
	-3, 6
};

static const s8int if_stage23_gain[] = {
	0, 3, 6, 9
};

static const s8int if_stage4_gain[] = {
	0, 1, 2, 2
};

static const s8int if_stage56_gain[] = {
	3, 6, 9, 12, 15, 15, 15, 15
};

static const s8int *if_stage_gain[] = {
	0,
	if_stage1_gain,
	if_stage23_gain,
	if_stage23_gain,
...
static const u8int if_stage_gain_len[] = {
	0,
...
static int find_stage_gain(u8int stage, s8int val)
{
	const s8int *arr;
	int i;
...
static const s8int if_gains_max[] = {
	0, 6, 9, 9, 2, 15, 15
};
...
int e4k_dc_offset_gen_table(struct e4k_state *e4k)
{
	...
	for (i = 0; i < ARRAY_SIZE(dc_gain_comb); i++) {
		u8int offs_i, offs_q, range, range_i, range_q;
		...
		fprint(2, "[E4K] Table %u I=%u/%u, Q=%u/%u\n",
